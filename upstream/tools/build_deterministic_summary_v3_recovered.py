# Reconstructed on 2026-09-18 from the supplied CPython 3.13 cache.
# See docs/UPSTREAM_RECOVERY.md; not an original historical source file.
...
RULE_VERSION = "summary_deterministic_from_agents_v3_molecular_microbiology"
...
def merge_filmarray_gmtest_payloads(
    *,
    ...
    no_filmarray: dict[str, Any] | None,
    raw_gm: Any,
) -> dict[str, Any] | None:
    if not isinstance(primary, dict) and not isinstance(no_filmarray, dict) and raw_gm is None:
        return None
    merged: dict[str, Any] = dict(primary) if isinstance(primary, dict) else {}
    if not merged and isinstance(no_filmarray, dict):
        merged = dict(no_filmarray)
    ...
    if isinstance(no_filmarray, dict):
        for item in labels_from_payload(no_filmarray):
            key = organism_label_key(item)
            if key and key not in seen:
                labels.append(item)
                seen.add(key)
                auxiliary_sources.append("filmarray_gmtest_agent_no_filmarray")
        for field in ("infection_likelihood", "probable_source", "probable_pathogen_type"):
            value = first_non_empty(merged.get(field), no_filmarray.get(field))
            if value is not None:
                merged[field] = value
...
def collect_hospital_organism_evidence(
    *,
    ...
    output = list(evidence_by_key.values())
    output.sort(
        key=lambda item: (
...
def build_summary(patient_dir: Path) -> dict[str, Any]:
    paths = {
        ...
        "filmarray_gmtest_no_filmarray": agent_path(patient_dir, "filmarray_gmtest_agent_no_filmarray"),
    ...
    payloads: dict[str, dict[str, Any] | None] = {
        ...
        if key not in {"filmarray_gmtest_no_filmarray", "raw_gm_test"}
    }
    no_filmarray_payload = (
        load_json(paths["filmarray_gmtest_no_filmarray"])
        if paths["filmarray_gmtest_no_filmarray"].exists()
        else None
    )
    raw_gm_payload = (
        load_json_any(paths["raw_gm_test"])
        if paths["raw_gm_test"].exists()
        else None
    )
    payloads["filmarray_gmtest"] = merge_filmarray_gmtest_payloads(
        primary=payloads.get("filmarray_gmtest"),
        no_filmarray=no_filmarray_payload,
    ...
    availability = module_availability(payloads)
    ...
    return {
        ...
        "cross_module_reasoning": [
            ...
            "D-SUM-01B: FilmArray/GM evidence also merges no-filmarray GM output and raw gm_test Aspergillus galactomannan detected results when primary agent output misses them.",
