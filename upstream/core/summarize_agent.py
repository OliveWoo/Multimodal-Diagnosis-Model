    ...
    *,
    ...
    specs.extend(
        [
            ...
            (
                "FilmArrayGMTestAgent",
                agent_dir / f"{base_name}_filmarray_gmtest_agent.json",
                True,
            ),
...
def _summary_mode_suffix(*, include_underlying: bool) -> str:
    return "with_underlying_with_filmarray" if include_underlying else "with_filmarray"
...
def derive_output_path(
    ...
    *,
    ...
    mode_suffix = _summary_mode_suffix(include_underlying=include_underlying)
...
def save_result(payload: dict, input_dir: Path, output_path: Path | None = None) -> Path:
    if output_path is None:
        output_path = derive_output_path(
            input_dir, include_underlying=False
        )
...
def summarize_directory(
    ...
    *,
    ...
    if include_underlying:
        logging.warning("include_underlying is deprecated and ignored; forcing include_underlying=False.")
...
def main() -> None:
    ...
    parser.set_defaults(include_underlying=False)
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing summary output files without prompting.",
    )
