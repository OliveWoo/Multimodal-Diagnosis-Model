...
from tools.source_provenance import attach_source_provenance
...
Preserve every microbiology result, organism/target, specimen or source, collection/report date,
quantity, unit, interpretation, imaging differential, diagnosis, and medication exposure found in
the input. Preserve repeated tests as separate records. Do not convert an imaging differential or
medication into microbiologic confirmation. Use null/Unknown for absent values; never invent them.
""".strip()
...
def save_result(
    ...
    output = attach_source_provenance(parsed, raw_payload, source_path) if raw_payload else parsed
...
def process_file(filepath: Path, output_dir: Path) -> None:
    ...
    if parsed is not None:
        output_path = save_result(parsed, filepath, output_dir, payload)
