...
def _contains_json_like_payload(text: str) -> bool:
    ...
    return '"raw_text"' in lowered or '"findings"' in lowered or '"confidence"' in lowered


def _source_file_label(source_file: Path) -> str:
    return source_file.name or str(source_file)
...
def _image_preprocessing(image: dict[str, Any]) -> dict[str, Any]:
    preprocessing = image.get("preprocessing")
...
def parse_slide(
    *,
    ...
    has_image_backed_text = bool(image_derived_text_blocks or image_findings)
    ...
    if not text_blocks and not has_image_backed_text:
        review_reasons.append("no_slide_text_detected")
    if not enriched_images:
        review_reasons.append("no_embedded_images_detected")
    if any(_image_preprocessing(image).get("suspicious_distortion") for image in enriched_images):
    ...
    if any(_contains_json_like_payload(block) for block in image_derived_text_blocks + image_findings):
        review_reasons.append("contains_unparsed_ocr_payload")
    ...
    return {
        "source_file": _source_file_label(source_file),
        ...
        "text_source_summary": {
            ...
            "image_text_block_count": len(image_derived_text_blocks),
            "image_finding_count": len(image_findings),
            "used_image_text_fallback": not text_blocks and has_image_backed_text,
        },
